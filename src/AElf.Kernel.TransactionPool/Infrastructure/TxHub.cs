    ...
    public class TxHub : ITxHub, ISingletonDependency
    {
        ...
        public async Task HandleTransactionsReceivedAsync(TransactionsReceivedEvent eventData)
        {
            foreach (var transaction in eventData.Transactions)
            {
                ...
                {
                    //Logger.LogWarning($"{receipt.TransactionId} - {eventData.Sender} - All Transactions already exists in TxStore");
                    continue;
                }
                ...
                if (txn != null)
                {
                    //Logger.LogWarning($"{receipt.TransactionId} - {eventData.Sender} - Transaction already exists in TxStore");
                    continue;
                }
